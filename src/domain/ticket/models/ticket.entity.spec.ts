...
describe("Ticket", () => {
    ...
    it('should throw DomainException when trying to cancel an unpaid ticket', async () => {
        const res = async () => {
            const ticket = new Ticket(
                TicketStatus.RESERVED,
                100,
        ...
        await expect(res).rejects.toThrow(ConflictDomainException);
        await expect(res).rejects.toThrow("Ticket can be cancelled only if it is paid");
    });
