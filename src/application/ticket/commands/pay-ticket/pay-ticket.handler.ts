...
import { NOTIFICATION_SERVICE_TOKEN, type NotificationService } from "src/application/extrenal-services/ports/notification.service";
...
export class PayTicketHandler implements ICommandHandler<PayTicketCommand> {
    ...
    public async execute({ ticketId, userId, token }: PayTicketCommand): Promise<void> {
        ...
        const result = await this.paymentService.createPayment(token, ticket.money.price);
        if (!result) throw new InternalServerErrorException("Server error");

        if (result.status !== "success") throw new ConflictException("Payment is failed");
