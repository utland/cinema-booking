...
export class CreateSessionReqDto {
    @ApiProperty({ example: "2026-06-10T14:00:00.000Z", type: String, format: "date-time", description: "Start time of the session" })
    @IsNotEmpty()
    @IsDate()
    @Type(() => Date)
    startTime: Date;

    @ApiProperty({ example: "2026-06-10T16:30:00.000Z", type: String, format: "date-time", description: "End time of the session" })
    @IsNotEmpty()
    @IsDate()
    @Type(() => Date)
    finishTime: Date;
    ...
    @ApiProperty({ example: "1f39c2f5-3b67-4b20-8c7a-ae9f1a6b2cfa", description: "Hall identifier where the session takes place" })
    @IsNotEmpty()
    @IsString()
    hallId: string;
}
