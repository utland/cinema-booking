...
export class TicketController {
    ...
    @Post()
    public async create(@Body() createTicketDto: CreateTicketApiDto, @CurrentUser() user: Payload): Promise<any> {
        const { sessionId, seatId, hallId } = createTicketDto;
        return await this.commandBus.execute(new CreateTicketCommand(sessionId, seatId, user.id, hallId));
    }

    @Patch("/cancel/:id")
    public async cancelTicket(@Param("id", new ParseUUIDPipe()) ticketId: string, @CurrentUser() user: Payload) {
        return this.commandBus.execute(new CancelTicketCommand(ticketId, user.id));
    }
    ...
    public async payTicket(
        ...
        return this.commandBus.execute(new PayTicketCommand(ticketId, user.id, token));
    }

    @Delete("/:id")
    public async remove(@Param("id", new ParseUUIDPipe()) ticketId: string, @CurrentUser() user: Payload) {
