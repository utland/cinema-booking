...
export class TicketController {
    ...
    @Post()
    public async create(@Body() createTicketDto: CreateTicketDto, @CurrentUser() user: IPayload) {
        return await this.ticketService.create(user.id, createTicketDto);
    }

    @Patch()
    public async update(@Body() updateStatusDto: UpdateTicketStatusDto, @CurrentUser() user: IPayload) {
        return this.ticketService.updateStatus(user.id, updateStatusDto);
    }

    @Delete("/:id")
    public async remove(@Param("id") id: string, @CurrentUser() user: IPayload) {
